function varargout = SU(varargin)
% LAMBDA = SU(N)
%   SU returns N-by-N generator matrices LAMBDA for the Special Unitary
...
%    172130RJUL2011
%
%  Added waitbars on 100842RSEP2011. (E. Blair)
%
time_SUstart = tic;
Nwait = 32;
Nsave = 0;
...
switch nargin
    case 0
        error('SU.M: Too few input arguments.')
    case 1
        N = varargin{1};
        V = eye(N);
    case 2
        ...
        V = varargin{2};
    otherwise
        error('SU.M: Too many input arguments.');
end % END:switch nargin

storefilename = ['SUdata_N', num2str(N), '.mat'];

S = N^2 - 1;

if N >= Nsave
    % check to see if previously-calculated data file is already stored
    stored_file_data = dir('SUdata_N*.mat');
    
    if length(stored_file_data) == 0 % No data has been saved
        disp('No SUdata_N*.mat files have been saved.');
        mustCalc = 1;
        ...
        if ismember(storefilename, NameList)
            disp(['SUdata_N', num2str(N), '.mat HAS been PREVIOUSLY SAVED.'])
            mustCalc = 0;
        else
            disp(['SUdata_N', num2str(N), '.mat has NOT been saved.'])
            mustCalc = 1;
        end
...
if mustCalc
    ...
    if N >= Nwait
        wbp = waitbar(0, 'SU.M: Please wait. Calculating P ...');
    end
    
    for a = 1:N
        for b = 1:N
            P{a,b} = outerprod(V(:,a), V(:,b));
        end % for b = 1:N
        if N >= Nwait
            waitbar(a/N, wbp)
        end
    ...
    uv_count = 1;
    if N >= Nwait
        wbuv = waitbar(0, 'SU.M: Please wait. Calculating u and v ...');
    end
    for k = 2:N
        for j = 1:k-1
    ...
    disp(['SU.M: Saved data to ''', storefilename, '''']);

    
else
    load(storefilename);
    disp(['SU.M: Data loaded from ''', storefilename, '''']);
end
...
disp(['SU(', num2str(N), ') took ', sec2hrstr(SUruntime)]);