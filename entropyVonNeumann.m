...
if isnan(SVN)
    ...
    disp(['ev = ', num2str(ev)]);
    disp(['max(imag(ev)) = ', num2str(max(imag(ev)))]);
    disp(['min(ev) = ', num2str(min(ev))]);
    disp(['max(ev) = ', num2str(max(ev))]);
    l2ev = log2(ev);
    disp(['log2(ev) = ', num2str(l2ev)]);
    
    [~, zero_ev_idx] = ismember(0, ev);
    disp(['Zero-valued eigenvalue indices: ', num2str(zero_ev_idx)]);
    nzeroev= length(zero_ev_idx);
    disp(['There are ' num2str(nzeroev), ' zero eigenvalues.'])
    for k = 1: nzeroev
        kqx = l2ev(zero_ev_idx(k));
        skqx = ev(zero_ev_idx(k))*kqx;
        disp(['v_k = ', num2str(kqx), '; s_k = ', ...
            num2str(skqx)]);
    end
